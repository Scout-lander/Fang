...
public class TrackableWeapon1 : Weapon
{
    ...
    protected override bool Attack(int attackCount = 1)
    {
        ...
        Vector3 spawnPosition = owner.transform.position + (Vector3)GetSpawnOffset(spawnAngle);

        // Find the closest enemy
        EnemyStats closestEnemy = FindClosestEnemy();
        if (closestEnemy != null)
        {
            Vector2 difference = closestEnemy.transform.position - spawnPosition;
            spawnAngle = Mathf.Atan2(difference.y, difference.x) * Mathf.Rad2Deg;
        }
